...
def parse_n_validate(statement=None):
    ...
    try:
        ...
        for _each_column in column_identifiers:
            if column_identifiers[_each_column]['table_name'] is None:
                for table_name in table_identifiers.keys():
                     if column_identifiers[_each_column]['column_name'] in _metadata[table_name]:
                        ...
                        column_identifiers[_each_column]['display_name']=table_name+'.'+column_identifiers[_each_column]['column_name']
            ...
            print "192",column_identifiers[_each_column]
            if 'function' not in column_identifiers[_each_column]: column_identifiers[_each_column]['function'] = None
...
def fetch_records(table_db=dict,column_db=dict):
    ...
    for _each_column in column_db:
        ...
        if constraint:
            constraints[_display_column]=' '.join(constraint)
...
#        _read_fp=pandas.read_csv(_table_name+'.csv', names = table_db[_each_table]['column_name'])
        if constraints:
            for _each_column in constraints:
                _constraint="_read_fp['%s'] %s" %(_each_column,constraints[_each_column])
                print _constraint
                #print _read_fp[_constraint]
...
def fetch_records_by_csv(table_db,column_db):
    for each_table in table_db:
        _read_fp = pandas.read_csv(table_db[each_table]['table_name'] + '.csv',names=table_db[each_table]['display_columns'])
        for _each_column in table_db[each_table]['display_columns']:
            _record= list(_read_fp['%s' % _each_column])
            table_db[each_table][_each_column] = _record
    #pprint(table_db)
    #pprint(column_db)
    _header=[]
    _records=[]
    for _each_column in column_db:
        _header.insert(column_db[_each_column]['order'], column_db[_each_column]['display_name'])
        _result=table_db[column_db[_each_column]['table_name']][column_db[_each_column]['display_name']]
        if column_db[_each_column]['constraint']:
            column_db[_each_column][column_db[_each_column]['display_name']]=[_each_element for _each_element in _result if eval('%s %s' %(_each_element,column_db[_each_column]['constraint']))]
        else:
            column_db[_each_column][column_db[_each_column]['display_name']]=_result
        _records.insert(column_db[_each_column]['order'],column_db[_each_column][column_db[_each_column]['display_name']])

    #print "After"
    pprint(column_db)
    print ('%10s'*len(_header) %tuple(_header))
    _records=zip(*_records)
    for _each in _records:
       print ('%10d'*len(_each) %(_each))
...
def main():
    table_db, column_db = parse_n_validate('selecT B from table1 where B>=200')
    fetch_records_by_csv(table_db,column_db)
